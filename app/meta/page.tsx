...
export default function MetaPage() {
  ...
  // Calculate remaining business days and required daily rate
  ...
  // Gauge percentage
  const gaugeRotation = Math.min(pctAtingido, 100) * 1.8; // 180 degrees max
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-foreground">Meta Mensal</h1>
      ...
      <div className="bg-card rounded-lg border border-border p-6 flex flex-col items-center">
        ...
        <p className="text-sm text-muted mt-2">da meta atingida</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <KPICard
          ...
          subtitle={`Para atingir a meta nos ${diasRestantes} dias restantes`}
          icon={Calculator}
          color="yellow"
        />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 px-3 text-muted font-medium">Dia</th>
                <th className="text-right py-2 px-3 text-muted font-medium">Meta Diária</th>
                <th className="text-right py-2 px-3 text-muted font-medium">Alcançado</th>
                <th className="text-right py-2 px-3 text-muted font-medium">Meta Acum.</th>
                <th className="text-right py-2 px-3 text-muted font-medium">Realizado Acum.</th>
                <th className="text-right py-2 px-3 text-muted font-medium">Diferença</th>
              </tr>
            </thead>
            <tbody>
              {dias.map((d, i) => {
                ...
                return (
                  ...
                  >
                    <td className="py-2 px-3 font-medium">Dia {d.dia}</td>
                    <td className="py-2 px-3 text-right">{formatBRL(d.metaDiaria)}</td>
                    <td className="py-2 px-3 text-right">
                      {d.alcancado > 0 ? formatBRL(d.alcancado) : "-"}
                    </td>
                    <td className="py-2 px-3 text-right">{formatBRL(d.metaAcumulada)}</td>
                    <td className="py-2 px-3 text-right">
                      {d.alcancadoAcumulado > 0 ? formatBRL(d.alcancadoAcumulado) : "-"}
                    </td>
                    <td
